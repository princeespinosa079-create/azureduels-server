...
const server = http.createServer((req, res) => {
  ...
  // POST /duelinvite — cross-server duel invite relay
  if (req.method === 'POST' && url === '/duelinvite') {
    getBody(req, (body) => {
      if (!body || !body.from || !body.to) { respond(res, 400, { error: 'Missing fields' }); return; }
      if (!notifs[body.to]) notifs[body.to] = [];
      notifs[body.to].push({ type: 'duelinvite', from: body.from, tm: Date.now() });
